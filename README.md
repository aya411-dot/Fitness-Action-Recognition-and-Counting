...
1. 视频预处理层：逐帧读取 → BGR 转 RGB → 送入 MediaPipe Pose 推理
2. 姿态估计层：MediaPipe Pose 输出 33 个 2D 骨骼关键点 → 归一化坐标转像素坐标 → 可见性过滤
3. 特征提取层：计算膝角、肘角、躯干角、平板直线度、躯干竖直夹角
4. 姿态粗分类层：基于躯干夹角将姿态分为站立 / 躺趴 / 未知
5. 动作判定层：根据姿态分支进入对应的状态机或计时器
6. 输出层：叠加骨架与计数文本，输出标注视频与 CSV 统计表
...
stateDiagram-v2
    [*] --> UP
    UP --> DOWN : 连续 N 帧 value 小于 down_th
    DOWN --> UP : 连续 N 帧 value 大于 up_th
```
