...
增强特性：本项目在基础关节角度识别方案之上，新增了以下三项关键改进：
...
┌───────────────────────────────────────────────────────────────────┐
│ 视频预处理层 │
│ 逐帧读取 → BGR→RGB 转换 → 送入 MediaPipe Pose 推理 │
└───────────────────────────────────────────────────────────────────┘
│
▼
┌───────────────────────────────────────────────────────────────────┐
│ 姿态估计层 │
│ MediaPipe Pose → 33 个 2D 骨骼关键点 │
│ 归一化坐标 → 像素坐标 → 可见性过滤（vis_th=0.25） │
└───────────────────────────────────────────────────────────────────┘
│
▼
┌───────────────────────────────────────────────────────────────────┐
...
│ tv < 50° → 站立（stand） │
│ tv > 55° → 躺/趴（lying） │
│ 50° ≤ tv ≤ 55° → unknown（不进入动作判定，避免误触发） │
└───────────────────────────────────────────────────────────────────┘
│
┌───────────────┼───────────────┐
▼ ▼ ▼
...
│ │
│ 叠加骨架 + 计数文本 → 输出标注视频 │
│ 写入 CSV 结构化统计表（Squat / Pushup / PlankTime） │
└───────────────────────────────────────────────────────────────────┘
...
给定三点 A、B、C，夹角 ∠ABC 定义为向量 BA 与 BC 之间的夹角：
A
...
为什么用点积 + acos：点积法天然抗象限翻转，输出稳定在 [0°, 180°] 区间；

数值裁剪：np.clip 处理浮点误差导致的超出 [-1, 1] 范围的边界情况。

2. 姿态粗分类（classify_pose）
数学定义：

给定肩中点 S、髋中点 H，定义躯干向量与图像竖直方向（y 轴向下）的夹角：

text
        torso_vec = H - S
        vertical_vec = (0, 1)

        torso_vert_angle = arccos( (torso_vec · vertical_vec) / (|torso_vec| · |vertical_vec|) )
分类规则：
...
动作	肘角范围	窗口极差	判定
前臂平板	83° ~ 91°	约 8°	Plank
俯卧撑	101° ~ 171°	约 70°	Pushup
直臂平板	165° ~ 175°	约 10°	Plank
参数说明：
...
评估指标
指标	计算方式	说明
计数准确率	1 - |预测次数 - 真实次数| / 真实次数	越接近 1 越好
平板时间误差	|系统计时 - 真实保持时长| / 真实时长	越接近 0 越好
误触发率	误判视频数 / 总视频数	越低越好
漏检率	未识别次数 / 实际次数	越低越好
实测结果
整体识别结果
...
plank（平板）	3	6~10s	6.2 / 9.1 / 9.1s	约 95%
pushup（俯卧撑）	3	3~5 次	2 / 4 / 4	约 85%
squat（深蹲）	3	2~10 次	2 / 8 / 2	约 90%
混淆矩阵
...
与基础关节角度识别方案的差异
模块	本项目	基础方案
姿态分类	躯干夹角 + 中间地带 unknown	简单 y 差阈值
俯卧撑/平板区分	滑动窗口稳定性判据	单帧肘角阈值
状态机防抖	连续 N 帧确认	单帧切换
平板计时	帧号计算 + 连续帧确认	POS_MSEC 时间戳
关键点容错	左右平均 + 可见性过滤	只用左侧关键点
角度计算	向量点积法	atan2 差值
...
选型理由：MediaPipe 模型体积小 10~20 倍，CPU 推理速度提升 6~10 倍，适合边缘部署和实时应用。
...
踩坑经历：初期用单帧肘角 > 140° 判为平板，导致俯卧撑撑起的瞬间被误判（撑起时肘角也接近 170°）。

解决方案：改用肘角滑动窗口极差，让"稳定性"成为核心判据。平板支撑的肘角在窗口内极差 < 10°，而俯卧撑的肘角在窗口内极差 > 60°。
...
原因分析：2D 投影下，肩-髋-踝在图像上不是一条直线（从正面看是上下分布的），躯干角度失真，torso_vert_angle 落在 30°~50° 的灰色地带。

最终方案：所有测试视频采用横屏侧拍 + 全身入镜，关键点可见性从 0% 提升到 95%+。

方法论启示：2D 姿态估计对拍摄角度敏感，实际部署时需明确使用场景约束。

已知局限
局限	原因	改进方向
仅支持单人	MediaPipe 只输出 1 组关键点	多人检测 + 关键点跟踪
...
未来工作
多动作扩展：基于骨架序列训练轻量级时序分类器，支持更多动作类型（弓步、引体向上等）；

多人场景：结合目标跟踪（ByteTrack / DeepSORT）实现多人独立计数；

3D 姿态估计：使用 MediaPipe Holistic 或 BlazePose GHUM 获取 3D 关键点，解决遮挡与视角问题；

实时部署：结合 ONNX Runtime 或 TensorRT 进一步加速，支持移动端实时推理；

动作质量评估：不仅计数，还评估动作标准度（如深蹲深度、俯卧撑肘角范围）。
...
Bazarevsky V, Grishchenko I, Raveendran K, et al. BlazePose: On-device Real-time Body Pose tracking[J]. arXiv preprint arXiv:2006.10204, 2020.

Cao Z, Hidalgo G, Simon T, et al. OpenPose: Realtime Multi-Person 2D Pose Estimation using Part Affinity Fields[J]. IEEE TPAMI, 2019.

Sun K, Xiao B, Liu D, et al. Deep High-Resolution Representation Learning for Visual Recognition[J]. IEEE TPAMI, 2019.
